...
class GreedyHunter(BaseAgent):
    ...
    def toroidal_manhattan(self, pos1, pos2):
        GridWidth = self.model.grid.width
        GridHeight = self.model.grid.height 
    ...
    def find_closest_prey(self):
        ...
        for prey in preys:
            dist = self.toroidal_manhattan(self.pos, prey.pos)
    ...
    def find_best_move_towards(self, target_pos, current_best_dist):
        ...
        for npos in neighbors:
            dist = self.toroidal_manhattan(npos, target_pos)
            if dist < best_dist:
                best_dist = dist
                best_move = npos
        return best_move
